  ...
  setup(__props) {
    ...
    const editbtn = () => {
      formRole.value.validate().then(async (res) => {
        if (alignmentFormData.value._id) {
          let data = await api_index.editlist({
            title: res.title,
            time: res.time,
            type: res.type,
            account: res.account,
    ...
    return (_ctx, _cache) => {
      return {
        ...
        q: common_vendor.sr(formRole, "ff3a2279-0", {
          "k": "formRole"
        }),
        r: common_vendor.p({
        ...
        s: common_vendor.o(editbtn)
      };
    };
  }
};
const MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["__scopeId", "data-v-ff3a2279"]]);
